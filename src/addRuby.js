        ...
        if (json.result.word[i].subword) {
            for (let n = 0; n < json.result.word[i].subword.length; n++) {
                if (/\p{sc=Han}/u.test(json.result.word[i].subword[n].surface)) {
                    ruby_list_part.push({ kanzi: json.result.word[i].subword[n].surface, furigana: json.result.word[i].subword[n].furigana })
                }
            }
        }
        else if (/\p{sc=Han}/u.test(json.result.word[i].surface)) {
            ruby_list_part.push({ kanzi: json.result.word[i].surface, furigana: json.result.word[i].furigana })
        }
        else if (json.result.word[i].surface === "<" && json.result.word[i+1].surface === ">") {
            ruby_list.push(ruby_list_part)
        }
    }
    return ruby_list
}
